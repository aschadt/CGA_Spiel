...
class ShaderProgram(vertexShaderPath: String, fragmentShaderPath: String) {
    ...
    private val matrixBuffer = MemoryUtil.memAllocFloat(16) // Füge das zu deinen Klassenattributen hinzu
    ...
    // Setzt eine mat4-Uniform im Shader-Programm
    fun setUniform(name: String, value: Matrix4f, transpose: Boolean = false): Boolean {
        // Wenn kein gültiges Shader-Programm aktiv ist, abbrechen
        if (programID == 0) return false

        // Hole die Speicheradresse (Location) der Uniform im Shader anhand ihres Namens
        val loc = GL20.glGetUniformLocation(programID, name)

        // Prüfe, ob die Uniform existiert (Location != -1 bedeutet: gefunden)
        if (loc != -1) {

            // Übertrage die Matrixdaten in den vorbereiteten FloatBuffer (OpenGL erwartet rohe float-Daten)
            value.get(matrixBuffer)

            // Übergibt die Matrix an die Uniform im Shader (transpose: Matrix transponieren oder nicht)
            GL20.glUniformMatrix4fv(loc, transpose, matrixBuffer)
            return true
        }
        return false
    }

    // Für int (z.B. sampler2D)
    fun setUniform(name: String, value: Int): Boolean {
        if (programID == 0) return false                        // Abbruch falls Shader-Programm nicht korrekt geladen wird
        val loc = GL20.glGetUniformLocation(programID, name)    // Abfrage wo sich die Uniform Name im aktiven Shader-Programm befinden
        if (loc != -1) {                                        // Wenn Uniform gefunden wurde, übergabe vom Wert
    ...
    fun setUniform(name: String, value: Boolean): Boolean {
        ...
        val loc = GL20.glGetUniformLocation(programID, name)
        if (loc != -1) {
            GL20.glUniform1f(loc, if(value) 1.0f else 0.0f)
            return true
        }
        return false
    }

    // Für Vector3f (z. B. Lichtfarbe oder Position)
